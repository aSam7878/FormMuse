...
Every catalog card includes a Catalog Teaser: a live, non-interactive view loaded from the template's isolated preview origin. It begins at the opening viewport and may run a deterministic template-specific preview-only sequence that automatically advances to a representative later view, such as the form. Render the card's semantic title, concise description, and destination link in the parent document immediately. The teaser is decorative supporting media: it must not enter the tab order, capture pointer input, duplicate the card's accessible name, or become the only way to understand or open the template.

Do not mount all 20 Complete Compositions during initial catalog render. Reserve stable teaser space and show a lightweight local fallback while inactive. Observe candidates with a 600px vertical margin, rank visible candidates before nearby candidates, and mount no more than three previews. Unmount a visited teaser after it leaves the active neighborhood and remount it from the deterministic opening state if it returns. If Intersection Observer is unavailable, retain the semantic card and local fallback without mounting a live preview. Respect reduced motion in teaser and full-preview contexts. Do not add pause/resume Preview Protocol commands for this lifecycle.

The Catalog Teaser begins at the template's opening viewport and, when the template defines one, may automatically move through a bounded preview-only sequence to show a representative later view. Hanging Gifts plays its opening entrance and advances automatically to the form while mounted. An off-screen teaser unmounts, and returning to it remounts the deterministic sequence from its opening state. Visitors cannot scroll or use the teaser as a miniature form. Selecting its surrounding card opens the Template Page, where the complete scrollable and interactive preview is available. Do not create separate teaser artwork or a second implementation of the design; both teaser and full preview render from the same template source and static preview infrastructure.
...
- Publish the production website, registry, and distinct preview origin only over HTTPS. Document and verify a host-appropriate security-header baseline—including CSP, MIME sniffing protection, referrer policy, permissions policy, and HSTS where applicable—without sacrificing the portable static export or isolated preview iframes.
