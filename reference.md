...
Every catalog card includes a Catalog Teaser: a live, non-interactive view loaded from the template's same-origin static preview route. It begins at the opening viewport and may run a deterministic template-specific preview-only sequence that automatically advances to a representative later view, such as the form. Render the card's semantic title, concise description, and destination link in the parent document immediately. The teaser is decorative supporting media: it must not enter the tab order, capture pointer input, duplicate the card's accessible name, or become the only way to understand or open the template.
