...
- Implements only the assigned issue in the isolated worktree.
...
4. The architect writes a worker brief that links the issue and relevant sources without copying the complete architecture into the prompt.
5. The worker implements and verifies the bounded change inside the worktree.
...
8. Corrections return to the same worktree with a precise review brief. The worker must not broaden scope while correcting findings.
