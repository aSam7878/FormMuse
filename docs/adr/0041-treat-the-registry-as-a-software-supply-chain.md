...
Production uses HTTPS for both the site and its distinct preview origin, with a host-appropriate reviewed security-header baseline compatible with the portable static export and isolated preview iframes. A root `SECURITY.md` with a private reporting path is required before public launch. Known vulnerabilities are triaged in context, and an unresolved relevant high- or critical-severity issue blocks publication or disables affected installation until remediated.
